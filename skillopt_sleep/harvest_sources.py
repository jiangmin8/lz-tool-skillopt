...
def harvest_for_config(cfg, *, since_iso: Optional[str] = None, limit: int = 0) -> list[SessionDigest]:
    ...
    if source == "custom":
        from skillopt_sleep.harvest_custom import harvest_custom
        trace_file = cfg.get("custom_trace_file", "")
        return harvest_custom(trace_file, since_iso=since_iso, limit=limit)
