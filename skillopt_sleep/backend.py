...
class AzureOpenAIBackend(CliBackend):
    """Drives Azure OpenAI gpt-5.x deployments via managed identity or API Key.
    ...
    Supports API Key auth via api_key parameter for local usage with OpenAI-compatible endpoints.
    """

    name = "azure"

    def __init__(self, deployment: str = "", endpoint: str = "", timeout: int = 180,
                 api_version: str = "2024-12-01-preview", api_key: str = "") -> None:
...
def get_backend(
    ...
    *,
    ...
    if n in {"azure", "azure_openai", "aoai"}:
        return AzureOpenAIBackend(deployment=model, endpoint=azure_endpoint, api_key=api_key)
...
def build_backend(
    *,
    ...
    if not has_split:
        be = get_backend(backend, model=model, codex_path=codex_path, azure_endpoint=azure_endpoint, api_key=api_key)
        be.preferences = preferences
        return be
    tgt = get_backend(target_backend or backend, model=target_model or model,
                      codex_path=codex_path, azure_endpoint=azure_endpoint, api_key=api_key)
